...
export function Header({locale}:{locale:Locale}) {
  ...
  return <header className="sticky top-0 z-50 border-b border-black/8 bg-[#f4f0e7]/92 backdrop-blur-xl">
    <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 md:px-8">
      <Link href={href("/")} className="font-display text-[1.65rem] font-black tracking-[-.06em]">TENTTOP<span className="text-olive">.</span></Link>
      <nav className="hidden items-center gap-6 text-sm font-semibold md:flex">{nav[locale].map(([n,h])=><Link key={h} href={href(h)} className="hover:text-olive">{n}</Link>)}<Link href={switchHref} className="rounded-full border border-black/15 px-3 py-2 text-xs font-black tracking-[.08em] hover:border-black/35" hrefLang={switchLocale}>{languageLabel}</Link><Link className="btn-dark" href={href("/contact")}>{contactLabel}</Link></nav>
      <div className="flex items-center gap-2 md:hidden"><Link href={switchHref} hrefLang={switchLocale} className="grid h-10 min-w-10 place-items-center rounded-full border border-black/15 px-2 text-xs font-black tracking-[.08em]">{languageLabel}</Link><button aria-label={locale==="es"?"Abrir menú":"Toggle menu"} onClick={()=>setOpen(!open)} className="grid h-11 w-11 place-items-center rounded-full border border-black/15">{open?<XIcon className="h-5 w-5"/>:<MenuIcon className="h-5 w-5"/>}</button></div>
    </div>
    {open&&<nav className="border-t border-black/10 bg-[#f4f0e7] px-5 py-4 md:hidden">{nav[locale].map(([n,h])=><Link key={h} onClick={()=>setOpen(false)} href={href(h)} className="block border-b border-black/8 py-4 text-lg font-semibold">{n}</Link>)}<Link onClick={()=>setOpen(false)} href={href("/contact")} className="mt-4 block btn-dark text-center">{contactLabel}</Link></nav>}
  </header>
}
