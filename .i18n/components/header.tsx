...
export function Header({locale}:{locale:Locale}) {
  ...
  return <header className="sticky top-0 z-50 border-b border-black/8 bg-[#f4f0e7]/92 backdrop-blur-xl">
    <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 md:px-8">
      ...
      <nav className="hidden items-center gap-6 text-sm font-semibold md:flex" aria-label={locale==="es"?"Navegación principal":"Primary navigation"}>{nav[locale].map(([n,h])=><Link key={h} href={href(h)} className="hover:text-olive">{n}</Link>)}<Link href={switchHref} className="rounded-full border border-black/15 px-3 py-2 text-xs font-black tracking-[.08em] hover:border-black/35" hrefLang={switchLocale} aria-label={languageAriaLabel}>{languageLabel}</Link><Link className="btn-dark" href={href("/contact")}>{contactLabel}</Link></nav>
      <div className="flex items-center gap-2 md:hidden"><Link href={switchHref} hrefLang={switchLocale} aria-label={languageAriaLabel} className="grid h-10 min-w-10 place-items-center rounded-full border border-black/15 px-2 text-xs font-black tracking-[.08em]">{languageLabel}</Link><button aria-label={open?(locale==="es"?"Cerrar menú":"Close menu"):(locale==="es"?"Abrir menú":"Open menu")} aria-expanded={open} aria-controls="mobile-navigation" onClick={()=>setOpen(!open)} className="grid h-11 w-11 place-items-center rounded-full border border-black/15">{open?<XIcon className="h-5 w-5"/>:<MenuIcon className="h-5 w-5"/>}</button></div>
    </div>
